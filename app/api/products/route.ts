...
async function fetchPage(pageNo = 1, numOfRows = 500) {
  ...
  try {
    ...
    // Fallback to text (XML) not implemented here; API is expected to support JSON when resultType=json
...
function readCsvFallback(): any[] {
  try {
    const csvPath = path.join(process.cwd(), "data", "KDIC_예금자보호_금융상품_전체목록.csv");
...
export async function GET() {
  try {
    ...
    try {
      ...
      // fallback to CSV if API or key is missing/failed
      data = readCsvFallback();
    }
